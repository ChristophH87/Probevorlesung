...
def plot_processes(r, rho, kappa, alpha, selected_takt):
    ...
    if selected_takt in ["Alle Takte", "Nur Wärmeabgabe"] and p4 is not None:
        if p1 is None:
            p1 = 1 / V1**kappa
    ...
    if selected_takt in ["Alle Takte", "Nur Wärmeabgabe"] and p5 is not None:
        if p1 is None:
            p1 = 1 / V1**kappa
        V_cool = np.ones(20) * V1
...
with st.sidebar:
    ...
    st.markdown("---")
    selected_takt = st.radio("🧭 Sichtbare Takte auswählen", [
        "Alle Takte",
        "Nur Kompression",
